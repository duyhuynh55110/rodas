...
locals {
  ...
  app_domain       = var.app_env == "prod" ? var.domain_name : "${var.app_env}.${var.domain_name}"
  app_admin_domain = var.app_env == "prod" ? "admin.${var.domain_name}" : "${var.app_env}-admin.${var.domain_name}"
  app_api_domain   = var.app_env == "prod" ? "api.${var.domain_name}" : "${var.app_env}-api.${var.domain_name}"
...
module "route53" {
  ...
  subdomains = {
    app : {
      ...
      type : "A"
      alias : {
        name : module.s3_web_hosting_bucket.cloudfront_domain_name
        zone_id : module.s3_web_hosting_bucket.cloudfront_hosted_zone_id
      }
    }
    admin : {
      name : local.app_admin_domain
      type : "A"
      alias : {
